...
class EveCheckpointError(RuntimeError):
    """Raised when the Suricata checkpoint is missing, corrupt, or unusable."""
...
def tail_file():
    ...
    while not _stop:
        try:
            ...
            # Fail closed: continuing with an in-memory cursor ahead of a
            # durable checkpoint (or after a corrupt one) would skip or
            # duplicate alerts across a restart.
            log.critical(
